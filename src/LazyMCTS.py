...
class LazyNode(object):
    ...
    def __init__(self, state, legalActions, priors, indices, id, **kwargs):
        self.State = state
        self.Value = 0
        self.Plays = 0
    ...
    def ChildWinRates(self):
        ...
        for i in range(len(self.Children)):
            if len(self._childWinRates) <= i:
                self._childWinRates = np.append(self._childWinRates, self.Children[i].WinRate())
            else:
    ...
    def ChildPlays(self):
        ...
        """
        for i in range(len(self.Children)):
            if len(self._childPlays) <= i:
                self._childPlays = np.append(self._childPlays, self.Children[i].Plays)
            else:
                self._childPlays[i] = self.Children[i].Plays
        return self._childPlays


class LazyMCTS(object):
    ...
    def AddChild(self, node, actionIndex):
        if node.Children is None:
            node.Children = []
        s = self._applyAction(node.State, actionIndex)
        priors, indices = self.GetPriors(s)
        node.Children.append(LazyNode(s, s.NumLegalActions(), priors, indices, actionIndex))
        node.Children[-1].Parent = node
        node._childIds.append(np.where(node.PriorIndices == actionIndex)[0][0])
    ...
    def FindMove(self, state, temp=0.1, moveTime=None, playLimit=None):
        ...
        if self.Root is None:
            priors, indices = self.GetPriors(state)
            self.Root = LazyNode(state, state.NumLegalActions(),
                             priors, indices, -1)
        ...
        realAction = self.Root.Children[self.Root._childIds.index(action)].id

        return (self._applyAction(state, realAction), self.Root.WinRate(),
    ...
    def _selectAction(self, root, temp, exploring=True):
        ...
        if exploring or temp == 0:
            ...
            upperConfidence = np.zeros(root.LegalActions)
            childWinRates = root.ChildWinRates()
            childPlays = root.ChildPlays()
            for i in range(len(upperConfidence)):
                if i in root._childIds:
                    upperConfidence[i] = childWinRates[root._childIds.index(i)] + (self.ExplorationRate * root.Priors[i] * np.sqrt(1.0+allPlays)) / (1.0 + childPlays[root._childIds.index(i)])
                else:
                    upperConfidence[i] = (self.ExplorationRate * root.Priors[i] * np.sqrt(1.0+allPlays))
            # upperConfidence = (root.ChildWinRates()
            #                    + (self.ExplorationRate *
            #                       root.Priors * np.sqrt(1.0 + allPlays))
            #                    / (1.0 + root.ChildPlays()))
            choice = np.argmax(upperConfidence)
            # unusedIds = self.GetFullChildIds(root)
            # for childId in root._childIds:
            #     unusedIds = np.delete(unusedIds, (unusedIds == childId))
            # otherConfidence = None
            # if len(unusedIds) > 0:
            #     otherConfidence = (self.ExplorationRate * root.Priors[unusedIds] * np.sqrt(1.0+allPlays))

            # if otherConfidence is None:
            #     choice = np.argmax(upperConfidence)
            # elif len(upperConfidence) == 0:
            #     choice = 4212 + unusedIds[np.argmax(otherConfidence)]
            # else:
            #     choice1 = np.argmax(upperConfidence)
            #     choice2 = np.argmax(otherConfidence)
            #     if upperConfidence[choice1] > otherConfidence[choice2]:
            #         choice = choice1
            #     else:
            #         choice = 4212 + unusedIds[choice2]
        else:
            allPlays = sum([p ** (1 / temp) for p in root.ChildPlays()])
            p = [c ** (1 / temp) / allPlays for c in root.ChildPlays()]
            choice = root._childIds[np.random.choice(len(root.ChildPlays()), p=p)]
    ...
    def GetPriors(self, state):
        ...
        """
        return np.array([1] * state.NumLegalActions()), np.where(state.LegalActions() == 1)[0]

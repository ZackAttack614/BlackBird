...
public:
    ...
    std::array<std::array<std::array<std::array<int, 15>, 8>, 8>, 1> AsInputArray() {
        std::array<std::array<std::array<std::array<int, 15>, 8>, 8>, 1> state = std::array<std::array<std::array<std::array<int, 15>, 8>, 8>, 1>();
        ...
        if (this->_black_castle_kingside) {
            for (int row = 0; row < 8; row++) {
                for (int col = 0; col < 8; col++) {
                    state[0][row][col][12] = 1;
                }
            }
        }
        if (this->_black_castle_queenside) {
            for (int row = 0; row < 8; row++) {
                for (int col = 0; col < 8; col++) {
                    state[0][row][col][13] = 1;
                }
            }
        }
        if (this->Player == 1 && this->PreviousPlayer == 1) {
            for (int row = 0; row < 8; row++) {
                for (int col = 0; col < 8; col++) {
                    state[0][row][col][14] = 1;
                }
            }
        }
        return state;
    ...
    py::array_t<T> py_InputArrayT(std::array<std::array<std::array<std::array<T, Z_DIM>, Y_DIM>, X_DIM>, W_DIM>& arr) {
        py::capsule freeWhenDone(&arr, [](void* p) {
            // No action needed as the std::array is managed externally
            });
        return py::array_t<T>(py::buffer_info(arr.data(), sizeof(int), py::format_descriptor<int>::format(), 4, { 1, 8, 8, 15 }, { 3840, 480, 60, 4 }));
        //return py::array_t<int>({ arr.size(), arr[0].size(), arr[0][0].size(), arr[0][0][0].size()}, arr.data());
    };
    py::array_t<int> py_InputArray() {
        auto arr = AsInputArray();
        return py_InputArrayT<int, 1, 8, 8, 15>(arr);
    }
...
// BOOST_PYTHON_MODULE(dragonchess) {
//     class_<BoardState>("BoardState", init())
//         .def("Board", &BoardState::Board)
//         .def("__str__", &BoardState::to_string)
//     ;
// }
const std::string BoardState::GameType = "DragonChess";
PYBIND11_MODULE(dragonchess, m) {
    py::class_<GameState> gamestate(m, "GameState");
    gamestate.def("EvalToString", &GameState::EvalToString);
    py::class_<BoardState>(m, "BoardState", gamestate)
        .def(py::init())
        .def("__str__", &BoardState::to_string)
        .def("LegalActions", &BoardState::py_LegalActions)
        .def("Winner", &BoardState::Winner)
        .def("AsInputArray", &BoardState::py_InputArray)
        .def("NumLegalActions", &BoardState::NumLegalActions)
        .def("ApplyAction", &BoardState::ApplyAction)
        .def("LegalActionShape", &BoardState::py_LegalActionShape)
        .def("Copy", &BoardState::Copy)
        .def("eq", &BoardState::eq)
        .def_property_readonly_static("LegalMoves", [](py::object) { return BoardState::NumLegalMoves;})
        .def_property_readonly_static("GameType", [](py::object) {return BoardState::GameType;})
        .def_readwrite("LastAction", &BoardState::LastAction)
        .def_readwrite("Player", &BoardState::Player)
        .def_readwrite("PreviousPlayer", &BoardState::PreviousPlayer);
}